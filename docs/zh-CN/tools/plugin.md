...
查看 [Plugin manifest](/plugins/manifest) 获取一个具体的插件示例。
...
- 独立文件：文件基础名（`~/.../my-plugin.ts` → `my-plugin`）
...
openclaw plugins install ./extensions/open-prose # 支持相对路径
openclaw plugins install ./plugin.tgz           # 从本地 tarball 安装
openclaw plugins install ./plugin.zip           # 从本地 zip 安装
openclaw plugins install -l ./extensions/open-prose # link（不复制），用于开发
openclaw plugins install @openclaw/open-prose # 从 npm 安装
openclaw plugins install @openclaw/open-prose --pin # 存储精确解析后的 name@version
...
- 主包：`openclaw`（本仓库）
- 插件：位于 `@openclaw/*` 下的独立 npm 包（例如：`@openclaw/open-prose`）
...
本 fork 不再包含 `voice-call` 语音通话插件；如果你需要一个轻量示例，
请参考仓库中的 `open-prose`、`browser` 或 `memory-core` 插件实现。
...
- 仓库内插件可以将 Vitest 测试放在 `src/**` 下（例如：`src/plugins/open-prose.plugin.test.ts`）。
