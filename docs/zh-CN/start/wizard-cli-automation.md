...
<AccordionGroup>
  ...
  <Accordion title="GitHub Copilot 示例">
    ```bash
    openclaw onboard --non-interactive \
      --mode local \
  ...
  <Accordion title="Google Gemini 示例">
    ```bash
    openclaw onboard --non-interactive \
      --mode local \
      --auth-choice gemini-api-key \
